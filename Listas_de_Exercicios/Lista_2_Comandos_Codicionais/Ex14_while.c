...
int main(){
    ...
    while (trab_lab < 0 || trab_lab > 10){
        printf("Nota invalida! Digite novamente (0 a 10): ");
        scanf("%f", &trab_lab);
        
    }
    printf("\nDigite a nota da avaliacao semestral (0 a 10): ");
    scanf("%f", &avali_semestral);

    while (avali_semestral < 0 || avali_semestral > 10){
        printf("Nota invalida! Digite novamente (0 a 10): ");
        scanf("%f", &avali_semestral);
    }
    printf("\nDigite a nota do exame final (0 a 10): ");
    scanf("%f", &exame_final);

    while (exame_final < 0 || exame_final > 10){
        printf("Nota invalida! Digite novamente (0 a 10): ");
        scanf("%f", &exame_final);
    }
    soma_pesos = peso1 + peso2 + peso3;
    media = (trab_lab * peso1 + avali_semestral * peso2 + exame_final * peso3) / soma_pesos;
    printf("\nA media ponderada do aluno é: %.2f\n", media);
